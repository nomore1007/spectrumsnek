    ...
    def decode_adsb_message(self, iq_samples: np.ndarray) -> List[Dict]:
        ...
        try:
            ...
            try:
                ...
                # For now, just note that pyModeS is available and pulses were detected
                # Real ADS-B decoding requires more complex signal processing
                if len(pulse_positions) >= 8:  # Minimum pulses for potential ADS-B
                    print(f"pyModeS available - detected {len(pulse_positions)} signal pulses", flush=True)
                    print("Real ADS-B decoding requires additional signal processing implementation", flush=True)
